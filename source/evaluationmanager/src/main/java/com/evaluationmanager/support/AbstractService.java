...
public abstract class AbstractService<T>{
	/**
	 * 数据操作的接口
	 * 该接口需要由子类确定，具体类类型由子类的getDataDao方法确定
	 */
	private DataDao<T> dataDao;
	/**
	 * @Author 陈彦磊
	 * @DateTime 2018年7月26日-上午10:50:06
	 * @Description 为数据操作的接口赋值，确定具体的操作对象
	 */
	public void setDataDao(DataDao<T> dataDao) {
	...
	public int changeData(T t,int operation){
		switch (operation) {
		case SAVE:
			return dataDao.add(t) ? SUCCESS : FAIL ;
		case UPDATE:
			return dataDao.update(t) ? SUCCESS : FAIL ;
		case DELETE:
			return dataDao.delete(t) ? SUCCESS : FAIL ;
		}
		return ERROR;
	}
	/**
